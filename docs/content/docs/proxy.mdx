...
Live runtime statistics plus two durable views:

- `persistent_savings` stores compression/cache savings in `~/.headroom/proxy_savings.json` (override with `HEADROOM_SAVINGS_PATH`).
- `request_history` is rebuilt from the append-only request JSONL configured by `--log-file`. It includes lifetime completed-request counts, provider/model breakdowns, tokens, latency, transforms, source coverage, malformed-line count, and time range.

For backward compatibility, `requests.total`, `tokens.*`, and `latency.*` remain current-runtime counters and now carry `scope: runtime`. Their durable counterparts are also available at `requests.lifetime`, `tokens.lifetime`, and `latency.lifetime`. `POST /stats/reset` resets only runtime counters and never deletes request history.
