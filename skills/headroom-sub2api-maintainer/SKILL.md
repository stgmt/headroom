...
- Use the paired skill's `scripts/rollout-sub2api-drained.ps1` for backend replacement. Its `/stats` probe is itself one active Headroom request, so safe idle is two consecutive `proxy_inbound.active <= 1` samples. On timeout it must fail closed without touching the live container; after replacement it must verify health and the exact expected revision.
