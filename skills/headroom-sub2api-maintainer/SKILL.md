...
- Cross-session incident map: `references/session-failure-registry.md`; canonical full registry in `stgmt/sub2api` at `backend/docs/skills/sub2api-claude-code-codex/references/session-failure-registry.md`
...
- User, system, and developer instructions are authoritative and must remain byte-exact in the paired Claude Code profile. `agent-90` may compress large tool/output material but must keep `compress_user_messages=false` and `compress_system_messages=false`; `protect_recent` is not a substitute because it only protects code-like content.
...
- A native subscription `429` with `Retry-After` must not terminate any Claude Code turn. When `HEADROOM_UPSTREAM_429_HOLD_ENABLED=1`, `stream:true` uses an HTTP 200 SSE hold with protocol-valid `ping` events; buffered `stream:false` uses an HTTP 200 chunked JSON hold with legal leading whitespace keepalives followed by the recovered JSON body. Both honor the full reset delay up to `HEADROOM_UPSTREAM_429_MAX_WAIT_SECONDS` and serialize response-header probes per upstream URL, credential, and requested model. A Sonnet/Opus cooldown must never hold GPT or Qwen sharing the same stable sub2api key, and any successful buffered or streaming response must clear only its matching stale model circuit. After ordinary short retries are exhausted, the same hold must cover configured transient `502/503/504/529` responses and transport disconnects. The Headroom library keeps a conservative 90-second transient default; the paired autonomous sub2api profile deliberately sets both the general and transient max wait to 86400 seconds because the general value caps the transient one, and therefore must also prove that WSL/container DNS and route self-heal can make progress. Do not apply it to an initially non-retryable `400/401` error, and do not silently enable another provider. Require `/health.runtime.upstream_recovery` counters plus cross-model isolation, successful-path cleanup, streaming and buffered 429, 503, transport, concurrent, cancellation, timeout, and repaired-route recovery tests; a green process-only upstream check is not provider-routability proof.
...
- Once Anthropic SSE output starts, telemetry cleanup cannot own the transport outcome. For F45, never rebind a captured `tags` parameter inside the nested async generator: derive `final_tags`, pass it to the finalizer, and catch finalizer telemetry errors without suppressing cancellation. Require executable clean-EOF and injected-failure tests, mutation proof, a Headroom-only rebuild, stable `proxy_inbound.active`, zero new ASGI errors, and live `claude --print`.
- For F46, a mid-response transport failure after `message_start` must not leak `event: error` to Claude Code. Keep text live, hold each `tool_use` block until `content_block_stop`, close a partial response with a valid Anthropic terminal checkpoint, and persist a one-shot marker keyed by `x-claude-code-session-id` plus agent id. The host `Stop`/`SubagentStop` hook consumes that marker and returns `decision:block`, which makes Claude Code issue the next model request in the same session. Cap the chain, clear it on a normal `message_stop`, and keep pre-`message_start` failures visible. Prove partial text, incomplete tool JSON, complete tool atomicity, clean EOF, endpoint consumption, hook loop guard, and a real fault-injected Claude run. Use the paired sub2api skill's `scripts/prove-claude-stream-recovery.ps1`; require `claude_exit=0`, exactly two upstream requests, one unique session id, and both `FIRST_PART` and `SECOND_PASS` in the transcript. Treat `console_checkpoint_events` as diagnostic because structured logs may not use stdout.
- For F47, never turn a live classified Claude Code `stream:true` request into a buffered `stream:false` CCR request. `headroom_retrieve` is server-private: skip its injection and strip an old sticky copy before forwarding the native SSE stream. Keep buffered CCR only for genuine client `stream:false` requests. Treat 429 reset holds separately from generic overloads: `HEADROOM_UPSTREAM_TRANSIENT_MAX_WAIT_SECONDS` caps 502/503/504/529, while 429 may honor the full reset. Prove the Claude stream stays native, non-Claude buffered CCR remains unchanged, and the two budgets differ.
- For F51, a long hold is not self-heal by itself. If sub2api reports `lookup chatgpt.com on 127.0.0.11:53: server misbehaving`, inspect WSL `/etc/resolv.conf` and the container resolver comments. `NO EXTERNAL NAMESERVERS DEFINED` means every retry is deterministic failure. The paired sub2api profile owns explicit compose DNS plus `repair-wsl-dns.ps1`; Headroom must keep the original request alive while that repair runs, then prove `recoveries_total` increased and the same Claude probe completed. Do not rebuild Headroom, rotate OAuth, or blame model quota for this class.
- For F49, distinguish Headroom forwarding silence from sub2api's Responses bridge silence with the shared request ID. If upstream sends only lifecycle frames, sub2api must emit Anthropic `ping` before `message_start` based on its last downstream write; that ping is transport liveness, not semantic output. It must leave the request eligible for one pre-output retry so an HTTP/2 body reset can switch only the matching upstream to H1. Do not buffer the whole answer, globally disable H2, or replay after text/tool output. Require the paired sub2api lifecycle-fault, delayed-success, retry-guard, mutation, rebuilt-image, and live Claude proofs.
- For F50, do not trust Docker `stop_grace_period` alone. The paired sub2api process must use `SUB2API_SERVER_SHUTDOWN_TIMEOUT=85s` with the profile's `90s` container grace; a shorter hardcoded application deadline can still kill healthy streams. Verify rendered config and the running image revision before a live Claude probe. This rollout guard is separate from Headroom recovery and F49 stream liveness.
- Use the paired skill's `scripts/rollout-sub2api-drained.ps1` for backend replacement. It passively tracks sub2api structured start/completion logs instead of perturbing Headroom `/stats`, pauses Headroom only after observed idle, rechecks the race while paused, recreates only sub2api, verifies health/revision, and always unpauses in `finally`. A pre-pause timeout must leave the live stack untouched.
...
python -m pytest tests/test_claude_stream_checkpoint_recovery.py tests/test_h2_stream_reset_retry.py
...
For native compact routing, the installed handler must contain `_is_claude_code_compact_request`, `x-sub2api-claude-compact`, and `headroom:claude_code_compact_prompt_preserved`. A real forked Claude Code `/compact`, not only a synthetic probe, must produce that transform marker in `proxy-requests.jsonl` and a Terra-medium/medium compact route in sub2api `usage_logs`. A tool or skill payload that merely mentions compaction must remain on the selected main route.
...
- Reuse the canonical `Fxx` IDs from `references/session-failure-registry.md`. New cross-stack classes are added to the sub2api registry first and mirrored here only when Headroom owns part of the mechanism.
