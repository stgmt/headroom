...
| `F37` | A real native Anthropic subscription cooldown or a temporary sub2api/network outage terminated all main and subagent turns although the containers stayed healthy. | Preserve each streaming turn with an HTTP 200 Anthropic SSE recovery hold; use pings, full `Retry-After`, route-scoped circuit/singleflight, and bounded retries for configured 429/502/503/504/529 plus transport failures. Keep 400/401 visible and never cross providers in `anthropic-only`. Prove health recovery counters and live ping-before-message recovery. |
