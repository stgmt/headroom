...
| `F45` | Upstream completed HTTP 200, but Headroom raised `UnboundLocalError ... 'tags'` after SSE output and leaked `proxy_inbound.active`. | Do not rebind the generator's input `tags`: build `final_tags`, pass it to telemetry, and catch telemetry-finalization errors after output. Prove full Anthropic event order through clean EOF, injected finalizer failure, killed mutations, stable active gap, zero ASGI errors, and live `claude --print`. |
| `F46` | A stream failed after visible assistant output; Headroom forwarded `event: error`, so Claude Code stopped the main/subagent turn and required user input. | Stream text live, hold incomplete `tool_use` frames, synthesize a legal terminal checkpoint after `message_start`, persist a bounded one-shot marker by Claude session plus agent, and let `Stop`/`SubagentStop` request the next turn. Keep pre-start failures visible. Prove fault injection through the real stack and two requests in one Claude transcript. |
| `F47` | A live Claude Code SSE request was internally converted to buffered JSON for private CCR retrieval; during upstream 502 it received JSON keepalives rather than Anthropic events and ended with `Stream idle timeout - no chunks received`. | Never inject or forward `headroom_retrieve` on a classified Claude Code `stream:true` request; preserve the native SSE path and retain buffered CCR only for real `stream:false` requests. Cap transient 502/503/504/529 holds separately from full 429 cooldowns. Prove the client stream stays true, private tool is absent, non-Claude buffered CCR still works, and the two wait budgets differ. |
